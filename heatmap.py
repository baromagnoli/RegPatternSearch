...
pivot_df = df.pivot_table(index='Organism', columns='Category', aggfunc='size', fill_value=0)
pivot_df = pivot_df.loc[species_order]

# Plotar heatmap
plt.figure(figsize=(12, 8))
sns.heatmap(pivot_df, annot=True, fmt='d', cmap='YlGnBu', cbar_kws={'label': 'Número de BGCs'})

# Melhorar layout
plt.xlabel('Categoria de BGC', fontsize=12)
plt.ylabel('Espécie', fontsize=12)
plt.title('Distribuição de BGCs por espécie', fontsize=14)
plt.tight_layout()

# Salvar figura
plt.savefig('bgcs_por_especie_heatmap.png', dpi=300)
plt.show()
